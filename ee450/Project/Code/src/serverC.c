...
open_transaction_file(FILE **fin){
  ...
  *fin = fopen("servers/block3.txt", "r+");
  if(*fin == NULL){
...
lookup_server_data(FILE **fin,
  ...
  printf("Looking up Server C data checking request type.\n");
  if(client_request_type == 1){
...
rewind_server_file(FILE **fin){
  if(0 != fseek(*fin, 0L, SEEK_SET)){
    perror("ERROR: Problem rewinding Server C file.\n");
    exit(1);
  }
}
...
int main(int argc, const char *argv[]){
  ...
  while(1){
	...
	printf("Appending transaction to Server C\n");
	...
	lookup_server_data(&fin,
      ...
      prep_and_send_udp_data(svr_c_sock_fd,
			     buf,
			     client_request_type,
			     username,
			     balance,
